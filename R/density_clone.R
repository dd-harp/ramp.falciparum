...
#' @export
d_clone_density = function(xi, a, FoI_a, bday=0,
                            hhat=1, r=1/200,
  ...
  clon_dens = function(x, a, FoI_a, bday, hhat, r, lRBC, m, W, par_Fmu, par_Omega){
    Pa = function(alpha, x, W, a, m){
      mu = Fmu(alpha, W, par_Fmu)
      y = zda(alpha, a, FoI_a, bday, hhat, r)
      d_Omega(x, mu, lRBC, par_Omega)*y/m
    }
    lowlim = 8
    uplim = min(a, 6*365)
    dff = stats::integrate(dAoI, lowlim, a, a=a, FoI_a=FoI_a,  bday=bday, hhat=hhat, r=r)$value
  ...
  if(length(xi)==1) return(clon_dens(xi, a, FoI_a, bday, hhat, r,
                                         lRBC, m, W, par_Fmu, par_Omega))
  return(sapply(xi, clon_dens, a=a, FoI_a=FoI_a,  bday=bday, hhat=hhat, r=r,
...
#' @return a [numeric] vector of length(x)
#' @export
moments_clone_density = function(a, FoI_a, bday=0, n=1, dt=0.1,
                                  hhat=1, r=1/200,
                                  par_RBC = par_lRBC_static(),
                                  par_Fmu=par_Fmu_base(),
                                  par_Omega = par_Omega_beta(),
  ...
  lRBC = log10RBC(a, par_RBC)
  xi = seq(0, lRBC, by = dt)
  Pd = d_clone_density(xi, a, FoI_a, bday, hhat, r,
